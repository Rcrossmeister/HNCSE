...
def cl_forward(cls,
    ...
    cos_sim_simple = cls.sim(z1.unsqueeze(1), z2.unsqueeze(0))
    cos_sim_simple = cos_sim_simple.to(cls.device)
    ...
    for i in range(batch_size):
        ...
        top_k_similarities = top_k_values[i]

        indices = top_k_indices[i]
        topK = z2[indices]
        G_lis = get_G(topK, top_k_similarities)
        G_lis = G_lis.to(z3.dtype)
        
        z3_G = torch.cat([z3, G_lis])
        cos_sim[i] = cls.sim(z1[i], z3_G.unsqueeze(0))
